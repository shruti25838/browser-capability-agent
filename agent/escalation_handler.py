...
class EscalationHandler:
    ...
    def handle(self, context: EscalationContext) -> EscalationResolution:
        ...
        # digit sequences) before anything here gets written to disk or console. The
        # screenshot path is protected -- it's a file reference, not observed content,
        # and its epoch-timestamp filename would otherwise itself get redacted away.
        record = redact_mapping(base_record, protected_keys=frozenset({"screenshot"}))

        # Log that the escalation happened BEFORE blocking on human input -- so every
        # screenshot taken always has a matching log entry, even if the prompt below
        # never returns (no interactive terminal, process killed mid-wait, etc.). An
        # orphaned screenshot with no log entry is itself a small bug.
        ...
        print(f"Reason stopped: {context.condition_type} failed after retries")

        try:
            answer = self.prompt(RESUME_PROMPT).strip().lower()
        except EOFError:
            ...
            self._append_log({**record, "human_action": "no_interactive_terminal", "resumed": False})
            return EscalationResolution(
                resumed=False,
                note="escalation triggered but no interactive terminal was available for human input",
            )
